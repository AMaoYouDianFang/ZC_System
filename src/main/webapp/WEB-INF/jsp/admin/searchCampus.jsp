                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button type="button" class="btn btn-danger btn-xs"
                                        onclick="remove('${item.stuid}','${pagingVO.curentPageNo}','${campusIndex}')">删除信息
                                </button>
                            </td>
                        </tr>
                    </c:forEach>
                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="/admin/searchCampus?page=1&campusid=${campusIndex}">首页</a></li>
                                <c:if test="${pagingVO.curentPageNo <= 1}">
                                    <li><a href="/admin/searchCampus?page=1&campusid=${campusIndex}">&laquo;上一页</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo > 1}">
                                    <li><a href="/admin/searchCampus?page=${pagingVO.upPageNo}&campusid=${campusIndex}">&laquo;上一页</a></li>
                                </c:if>
                                </li>
                                <li class="active"><a href="">${pagingVO.curentPageNo}</a></li>
                                <c:if test="${pagingVO.curentPageNo+1 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/admin/searchCampus?page=${pagingVO.curentPageNo+1}&campusid=${campusIndex}">${pagingVO.curentPageNo+1}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+2 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/admin/searchCampus?page=${pagingVO.curentPageNo+2}&campusid=${campusIndex}">${pagingVO.curentPageNo+2}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+3 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/admin/searchCampus?page=${pagingVO.curentPageNo+3}&campusid=${campusIndex}">${pagingVO.curentPageNo+3}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+4 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/admin/searchCampus?page=${pagingVO.curentPageNo+4}&campusid=${campusIndex}">${pagingVO.curentPageNo+4}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo <pagingVO.totalCount}">
                                    <li><a href="/admin/searchCampus?page=${pagingVO.nextPageNo}&campusid=${campusIndex}">下一页&raquo;</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo >=pagingVO.totalCount}">
                                    <li><a href="/admin/searchCampus?page=${pagingVO.totalCount}&campusid=${campusIndex}">下一页&raquo;</a></li>
                                </c:if>
                                <li><a href="/admin/searchCampus?page=${pagingVO.totalCount}&campusid=${campusIndex}">尾页</a></li>
...
<script type="text/javascript">
    ...
    function jumpPage(){
        ...
        if(page<=${pagingVO.totalCount}){
            window.location.href="/admin/searchCampus?page="+ page + "&campusid=" +${campusIndex};
        }
    }
    ...
    function remove(id,page,campusid) {
        var msg = "您确定要删除吗";
        if (confirm(msg) == true) {
            window.location.href="/admin/removeStuCampus?stuid=" + id + "&currentPage=" +page +"&campusid=" +campusid;
            return true;
        } else {
            return false;
        }
