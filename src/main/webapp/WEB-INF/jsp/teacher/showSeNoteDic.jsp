                        ...
                        <tr>
                            ...
                            <th>
                                <button type="button" class="btn btn-primary btn-xs"
                                        onClick="window.open('/teacher/editSeNoteTable?textid=${item.textid}')">查看表格
                                </button>
                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="/teacher/showSeNoteDic?page=1">首页</a></li>
                                <c:if test="${pagingVO.curentPageNo <= 1}">
                                    <li><a href="/teacher/showSeNoteDic?page=1">&laquo;上一页</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo > 1}">
                                ...
                                <c:if test="${pagingVO.curentPageNo <pagingVO.totalCount}">
                                    <li><a href="/teacher/showSeNoteDic?page=${pagingVO.nextPageNo}">下一页&raquo;</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo >=pagingVO.totalCount}">
                                    <li><a href="/teacher/showSeNoteDic?page=${pagingVO.totalCount}">下一页&raquo;</a></li>
                                </c:if>
                                <li><a href="/teacher/showSeNoteDic?page=${pagingVO.totalCount}">尾页</a></li>
...
<script type="text/javascript">
    ...
    function jumpPage(){
        ...
        if(page<=${pagingVO.totalCount}){
            window.location.href="/teacher/showSeNoteDic?page="+ page;
        }
    }
    ...
    function remove(textid,page) {
        var msg = "您确定要删除吗";
        if (confirm(msg) == true) {
            window.location.href="/teacher/removeSeNoteDic?textid=" + textid + "&currentPage=" +page;
            return true;
        } else {
            return false;
        }
